...
export class ImageExportComponent {
  ...
  readonly displayGeoNote = computed(() => (this.hasData() ? this.geoNote() : GUILT_TRIPPING_NOTE));
...
function drawExportCard(
  ...
  if (stats.activityType) {
    ...
    context.fillStyle = accent.color;
    context.fillText(activityText, pillX + paddingX, pillY + pillHeight / 2 + 7);
  }
